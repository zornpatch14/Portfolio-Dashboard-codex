...
def parse_tradestation_trades(file_bytes: bytes, filename: str) -> pd.DataFrame:
    ...
    if not out.empty:
        ...
        for c in ["net_profit", "runup", "drawdown_trade", "contracts", "commission", "slippage", "CumulativePL_raw"]:
            ...
            pretty_card([
                ...
                dcc.Tabs(
                    ...
                    children=[
                        ...
                        dcc.Tab(label="Net Contracts", value="netpos"),
                    ],
                ),
...
def build_net_contracts_figure(store_trades: dict,
    ...
    # Portfolio series (sum across everything)
    portfolio_series = make_series(ev[["ts", "delta", "kind_rank"]])
    ...
    fig.update_layout(
        title="Net Contracts Over Time (absolute; direction in hover)",
...
def update_analysis(store_trades, selected_files, selected_symbols, selected_intervals, selected_strategies, direction, start_date, end_date, active_tab):
    ...
    elif active_tab == "netpos":
        chart_fig = build_net_contracts_figure(
    ...
    for f, tdf in trades_by_file.items():
        ...
        m = compute_metrics(tdf, DEFAULT_INITIAL_CAPITAL)
        row.update(m)
        rows.append(row)

    # Portfolio row
    if eq_all is not None and "Portfolio" in eq_all and not eq_all.empty:
        # Build a single DF representing the union of selected trades
        pf_trades_list = []
        for f, tdf in trades_by_file.items():
            if tdf is not None and not tdf.empty:
                pf_trades_list.append(tdf.assign(File=f))
        pf_trades = pd.concat(pf_trades_list, ignore_index=True) if pf_trades_list else pd.DataFrame()
    ...
    # Columns order (ensure important ones visible first)
    col_order = [
        ...
        "Max. Trade Drawdown",
        # Portfolio-level (primarily meaningful for 'Portfolio' row)
...
def export_trades(n_clicks, store_trades, sel_files, sel_syms, sel_ints, sel_strats, direction, start_date, end_date):
